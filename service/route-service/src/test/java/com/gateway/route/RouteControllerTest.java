...
import static com.gateway.route.RouteController.FORWARDED_URL;
import static com.gateway.route.RouteController.PROXY_METADATA;
import static com.gateway.route.RouteController.PROXY_SIGNATURE;
...
public class RouteControllerTest {
    ...
    @Test
    public void getRedirectRequest() throws Exception {
        this.mockServer
                .expect(method(GET))
                .andExpect(requestTo("http://localhost/redirect/get"))
                .andExpect(header(PROXY_METADATA, PROXY_METADATA_VALUE))
                .andExpect(header(PROXY_SIGNATURE, PROXY_SIGNATURE_VALUE))
                .andRespond(withSuccess(BODY_VALUE, TEXT_PLAIN));
        ...
        this.mockMvc
                .perform(get("http://localhost/route-service/get")
                        .header(FORWARDED_URL, "http://localhost/forward/get")
                        .header(PROXY_METADATA, PROXY_METADATA_VALUE)
                        .header(PROXY_SIGNATURE, PROXY_SIGNATURE_VALUE))
                .andExpect(status().isOk())
                .andExpect(content().contentType(TEXT_PLAIN))
